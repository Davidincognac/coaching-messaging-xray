...
def score_leadcapture(r):
    """6. Lead capture that actually helps the buyer, not just 'join my newsletter'."""
    text = " ".join([s(r.body_text), s(r.h2_headings), s(r.cta_text)]).lower()
    magnet = any_kw(text, LEAD_MAGNET_KEYWORDS)          # a freebie that solves a real problem + pulls into the funnel
    newsletter = any(k in text for k in NEWSLETTER_KW)
    optin = s(r.optin_present) == "yes"
    if magnet:
        return 8
    if newsletter or optin:
        return 3                                        # value to the coach, not the buyer
    return 0
...
# A money figure only counts as PRICING when it sits in a price context, a cadence ('per session', '/month') or a
# price cue ('investment', 'packages from £'). A stray currency figure in a client RESULT ('grew revenue by $500k')
# or a discount ('$50 off') is NOT the cost of working with the coach, so it must not score pricing 10.
_MONEY_FIG_RE = re.compile(r"[£$€]\s?\d[\d,]*(?:\.\d+)?")
# A PRICE noun near the figure ('investment $2,000', 'packages from £') is an unambiguous price.
_PRICE_NOUN_RE = re.compile(
    r"\binvest(?:ment)?\b|\bpricing\b|\bpriced?\b|\bcost[s:]?\b|\bfee[s:]?\b|\bpackages?\b|\btuition\b|"
    r"\bpayment plan\b|\bdeposit\b|\bretainer\b|\benrol|\bsubscription\b|membership (?:is|costs?|starts)|"
    r"\bbilled\b|(?:starting at|starts at|priced at)\s*[£$€]", re.I)
# A payment CADENCE ('$150 per session', '$99/month') is a price ONLY if no RESULT word is nearby: coaching results
# are quoted the same way ('$150,000 monthly revenue', '$30k/month in sales'), so a result noun in the window vetoes it.
_CADENCE_RE = re.compile(r"(?:per|/|a)\s?(?:month|mo|week|session|hour|hr|year|class)\b|\b(?:monthly|weekly|hourly)\b", re.I)
_RESULT_NOUN_RE = re.compile(
    r"\brevenue\b|\bincome\b|\bsales\b|\bprofit\b|\bearn|\bgr[eo]w\b|\bgrowth\b|\bgenerated?\b|\bturnover\b|\bmade\b|"
    r"\braised\b|\bincreased?\b|\bscaled?\b|\bwent from\b|\bfrom \$?\d", re.I)

def score_pricing(r):
    """9. Pricing posture: is the actual COST of working with the coach shown? Only a money figure in a real PRICE
    context counts, never a client-RESULT number ('$150,000 monthly revenue', '$500k'), which used to mint a false 10."""
    body = s(getattr(r, "body_text", "")) or s(getattr(r, "pricing_mentions", ""))
    if not body:
        return 0
    for m in _MONEY_FIG_RE.finditer(body):
        win = body[max(0, m.start() - 45): m.end() + 45]
        if _PRICE_NOUN_RE.search(win):
            return 10
        if _CADENCE_RE.search(win) and not _RESULT_NOUN_RE.search(win):
            return 10
    return 0
...
CRITERIA = [
    ...
    ("proof", score_proof),
    ("clear_cta", score_cta),
    ("lead_capture", score_leadcapture),
...
WEIGHTS = {
    ...
    "story": 1.0, "credibility": 1.0, "lead_capture": 1.0,
    "clear_cta": 0.5, "technical_health": 0.3, "pricing_shown": 0.2,
}
_WSUM = sum(WEIGHTS.values())

def weighted_total(scores):
    """Overall 0-100, weighting persuasion above mechanics."""
    return round(10 * sum(scores.get(k, 0) * w for k, w in WEIGHTS.items()) / _WSUM, 1)
...
def main():
    ...
    lines.append(f"- **{pct(home['lead_capture'] == 0)}% capture nothing** — no opt-in, no lead magnet, so the ~97% not ready to buy just leave.")
