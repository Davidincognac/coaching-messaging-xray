...
_PRICE_CUE_RE = re.compile(
    r"(?:per|/)\s?(?:month|mo|week|session|hour|hr|year)\b|\b(?:monthly|weekly|hourly)\b|"
    r"\bper (?:month|session|week|hour|year|class)\b|"
    r"\binvest(?:ment)?\b|\bpricing\b|\bprice[d:]?\b|\bcost[s:]?\b|\bfee[s:]?\b|\bpackages?\b|\btuition\b|"
    r"\bpayment plan\b|\bdeposit\b|\bretainer\b|\benrol|(?:starting at|starts at|priced at)\s*[£$€]", re.I)
...
def score_pricing(r):
    ...
    context counts, never a client-result number ('$500k revenue') or a discount, which used to mint a false 10."""
    body = s(getattr(r, "body_text", "")) or s(getattr(r, "pricing_mentions", ""))
    if not body:
        return 0
    for m in _MONEY_FIG_RE.finditer(body):
        if _PRICE_CUE_RE.search(body[max(0, m.start() - 40): m.end() + 40]):
            return 10
    return 0
