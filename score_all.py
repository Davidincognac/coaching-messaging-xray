...
def score_specificity(r):
    """2. FOCUS: does the headline name a single specific audience AND a single specific problem? Generic = 1.
    NOTE: this is a headline-only corpus proxy used for BENCH stats. It rewards a focused headline but cannot see
    BODY-level breadth (a page that lists five different problems lower down). The LIVE tool's specificity score comes
    from the AI rubric in audit.py, which now CAPS breadth (broad appeal / general life coaching / several audiences
    or problems at once) at 3 — that rubric is the authority on breadth. The BENCH market average won't reflect the
    stricter breadth rule until the corpus is re-scored, so the 'market' figure shown for specificity runs generous."""
    head = _headline(r)
    who = has_audience(head)
    what = has_concrete(head)
    if who and what:
...
def score_booking(r):
    ...
    from the total). Scored by TYPE, not free vs paid: direct booking 8, contact/enquiry form 4, application 3.
    (Corpus is text-only, so a link-only 'Book' button can't be seen here; the live tool also reads button links.)"""
    text = " ".join([s(r.body_text), s(r.h2_headings), s(r.cta_text)]).lower()
